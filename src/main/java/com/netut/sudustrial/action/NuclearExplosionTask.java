...
public class NuclearExplosionTask {
    ...
    // Ширина приграничной полосы (в блоках), где вообще имеет смысл проверять соседей
    // на "живую" воду снаружи радиуса — вдали от края сферы соседи гарантированно тоже
    // внутри, проверять там нечего, это чистая экономия лишних чтений блоков.
    private static final int SURFACE_MARGIN = 2;

    // Чем "затыкаем" настоящую протечку с внешним океаном/рекой.
            ...
            for (int ly = 0; ly < 16; ly++) {
                ...
                for (int lz = 0; lz < 16; lz++) {
                    ...
                    if (distSq <= rSq) {
                        ...
                        if (current.is(ModBlocks.NUCLEAR_AIR) || current.is(Blocks.BEDROCK) || current.isAir()) continue;

                        // Только у самого края сферы проверяем: не граничит ли эта клетка
                        // с настоящей, нетронутой водой снаружи радиуса. Если да — сразу
                        // затыкаем её камнем вместо воздуха, ещё до того как вода вообще
                        // успела бы туда потечь.
