...
import com.netut.sudustrial.entity.CustomTntEntity;
...
public class Explodes {
    ...
    public static void explode(CustomTntEntity entity) {
        ...
        if (entity.isMining()) {
            ...
            if (power == 0.0f) {
                ...
                // Если мощность > 100.0F — запускаем оптимизированный взрыв без лагов UBO и чанков
                if (power > 32.0F) {
    ...
    public static void nuclearExplode(ServerLevel level, CustomTntEntity entity, NuclearTntType type) {
        ...
        // Ядро ставим СРАЗУ — радиация начинает действовать с этого же тика,
        // а не после того как кратер (чистая косметика) дорисуется целиком.
        ...
        // Кратер — фоновая косметика, растёт постепенно по тикам
    ...
    public static void spawnerExplode(ServerLevel level, CustomTntEntity entity, SpawnerTntType type) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            // Если у вас версия ниже 1.21.2, закройте EntitySpawnReason.TRIGGERED на MobSpawnType.TRIGGERED
    ...
    public static void effectExplode(ServerLevel level, CustomTntEntity entity, EffectTntType type) {
        ...
        float maxRadius = type.getRadius(); // Например, 5.0F
        int maxRadiusInt = (int) Math.floor(maxRadius);
        int maxDuration = type.getDuration(); // Общая длительность центрального слоя

        // Скорость сжатия центрального (самого большого) диска в блоках за тик
        ...
        for (int yOffset = -maxRadiusInt; yOffset <= maxRadiusInt; yOffset++) {
            ...
            // Маленькие кольца сверху и снизу проживут меньше и исчезнут раньше!
            ...
            // Все кольца уменьшаются с ЕДИНОЙ абсолютной скоростью (shrinkSpeed)
    ...
    public static void miningExplode(ServerLevel level, CustomTntEntity entity, MiningTntType type) {
        ...
        if (type.isColumn()) {
            // Режим 2: Прямоугольная колонна вниз до Bedrock
